...
export class DialogReceptionistComponent implements OnInit {
	...
	async onSubmit() {
		...
		try {
			...
			if (receptionist.id) {
				await this._receptionistService.update(receptionist, {
					picture: this.picture,
				});
				this._alertService.handleSuccess('Atualizado com sucesso!');
			} else {
				await this._receptionistService.save(receptionist, {
					picture: this.picture,
				});
	...
	private _buildForm() {
		this.form = this._formBuilder.group({
			...
			cpf: [this.receptionist?.cpf, [Validators.required]],
			birthDate: [
				this.receptionist?.birthDate
					? new Date(this.receptionist.birthDate + 'T00:00:00')
					: null,
				[Validators.required],
			],
			phone: [this.receptionist?.phone, [Validators.nullValidator]],
			status: [this.receptionist?.status, [Validators.required]],
			maritalStatus: [
				this.receptionist?.maritalStatus,
				[Validators.nullValidator],
			],
			gender: [this.receptionist?.gender, [Validators.required]],
