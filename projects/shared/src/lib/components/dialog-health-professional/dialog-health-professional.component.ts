...
export class DialogHealthProfessionalComponent implements OnInit {
	...
	async ngOnInit(): Promise<void> {
		...
		this.medicalSpecialtyOptions = Object.values(MedicalSpecialty).map(
			(value) => ({
	...
	async onSubmit() {
		...
		try {
			...
			if (healthProfessional.id) {
				await this._healthProfessionalService.update(
					healthProfessional,
					{
				...
				await this._healthProfessionalService.save(healthProfessional, {
					picture: this.picture,
				});
			}
	...
	private _buildForm() {
		this.form = this._formBuilder.group({
			...
			cpf: [this.healthProfessional?.cpf, [Validators.required]],
			birthDate: [
				this.healthProfessional?.birthDate
					? new Date(this.healthProfessional.birthDate + 'T00:00:00')
					: null,
				[Validators.required],
			],
			phone: [this.healthProfessional?.phone, [Validators.nullValidator]],
			status: [this.healthProfessional?.status, [Validators.required]],
			maritalStatus: [
				this.healthProfessional?.maritalStatus,
				[Validators.nullValidator],
			],
			gender: [this.healthProfessional?.gender, [Validators.required]],
			...
			rg: [this.healthProfessional?.rg, [Validators.nullValidator]],
			rgIssuer: [
				this.healthProfessional?.rgIssuer,
				[Validators.nullValidator],
			],
			cns: [this.healthProfessional?.cns, [Validators.nullValidator]],
			councilType: [
