...
export class FileUtils {
     ...
     */
    static async toFileLoaded(file: File, options?: { maxSize?: number; types?: string[] }): Promise<FileLoaded | null> {
        const maxFileSize = options?.maxSize ? options.maxSize * 1024 * 1024 : Infinity;
        const isTypeAllowed = !options?.types || options.types.some(type => file.type.startsWith(type));
     ...
     * @returns {Promise<Media[]>} A promise that resolves to an array of Media objects.
     */
    static async toFilesLoaded(files: FileList, options?: { maxSize?: number; types?: string[] }): Promise<FileLoaded[]> {
        ...
        return resolvedFiles
            .filter((fileLoaded): fileLoaded is FileLoaded => fileLoaded !== null)
