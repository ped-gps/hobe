...
export class MedicalAppointmentService extends AbstractService<MedicalAppointment> {
		...
		return new Promise<MedicalAppointment>((resolve, reject) => {
			this._http
				.get<MedicalAppointment>(
					`${this._baseURL}/appointment/${appointmentId}`,
				)
				.subscribe({
	...
	overview(healthProfessionalId: string, startDate: string, endDate: string) {
		return new Promise<MedicalAppointmentOverview>((resolve, reject) => {
			const params: any = { healthProfessionalId, startDate, endDate };

			this._http
				.get<MedicalAppointmentOverview>(`${this._baseURL}/overview`, {
					params: params,
				})
				.subscribe({
					next: (response) => {
					...
					error: (error) => {
						...
						this._alertService.showMessage(
							AlertType.ERROR,
							'Erro',
							'Não foi possível obter os dados estatísticos dos atendimentos!',
						);
						reject(error);
					},
				});
	...
	printDocumentsCertificates(
		id: string,
		payload: MedicalAppointmentDocumentPrintRequest,
	): Promise<Blob> {
		return new Promise<Blob>((resolve, reject) => {
			this._http
				.post(`${this._baseURL}/${id}/documents/print`, payload, {
	...
	printExtract(
		requestBody: MedicalAppointmentExtractPrintRequest,
	): Promise<Blob> {
		return new Promise((resolve, reject) => {
			this._http
				.post(`${this._baseURL}/extract/print`, requestBody, {
					headers: new HttpHeaders({
						'Content-Type': 'application/json; charset=utf-8',
						Accept: 'application/pdf',
					}),
	...
	printPrescribedExams(
		id: string,
		payload: PrescribedExamsPrintRequest,
	): Promise<Blob> {
		return new Promise<Blob>((resolve, reject) => {
			this._http
				.post(
					`${this._baseURL}/${id}/prescription/exams/print`,
					payload,
					{
						headers: new HttpHeaders({
							'Content-Type': 'application/json; charset=utf-8',
	...
	printPrescribedMedications(
		id: string,
		payload: PrescribedMedicationsPrintRequest,
	): Promise<Blob> {
		return new Promise<Blob>((resolve, reject) => {
			this._http
				.post(
					`${this._baseURL}/${id}/prescription/medications/print`,
					payload,
					{
						headers: new HttpHeaders({
							'Content-Type': 'application/json; charset=utf-8',
	...
	printPrescribedVaccines(
		id: string,
		payload: PrescribedVaccinesPrintRequest,
	): Promise<Blob> {
		return new Promise<Blob>((resolve, reject) => {
			this._http
				.post(
					`${this._baseURL}/${id}/prescription/vaccines/print`,
					payload,
					{
						headers: new HttpHeaders({
							'Content-Type': 'application/json; charset=utf-8',
		...
		return new Promise<MedicalAppointment>(async (resolve, reject) => {
			try {
				...
				if (files.length) {
					...
					await this._http
						.put(
							`${this._baseURL}/${updated.id}/attachments`,
							formData,
							{
								reportProgress: true,
								observe: 'events',
